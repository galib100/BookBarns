...
import styles from "./BlogEditModal.module.css";

import { BASE_URL } from "../../../Constants/URL";

const BlogEditModal = ({ blogEditModalToggleAction, open, editBlog, data }) => {
  ...
  const handleClose = () => {
    setSelectedFile(undefined);
    setPreview(undefined);
    blogEditModalToggleAction(data._id);
  };

  const onSubmitHandeler = (values) => {
    //Blog ACTION CALL
    let flag = editBlog(values, selectedFile, data._id);
    if (flag) {
      setSelectedFile(undefined);
      setPreview(undefined);
      blogEditModalToggleAction();
      swal("Blog Edited", "", "success");
    } else {
      swal("Error", "", "error");
    }
  };
  ...
  if (data) {
    initVals.title = data.title ? data.title : "";
    initVals.description = data.short_description ? data.short_description : "";
  }
...
const mapStateToProps = (state) => ({
  open: state.admin_blog_page.edit_modal,
  data: state.admin_blog_page.selected_blog,
});
